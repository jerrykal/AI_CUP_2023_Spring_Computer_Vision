...
import pandas as pd
import torch
from dataset import BackhandDataset, load_features

from model import BackhandClassifier


def main(args):
    model = BackhandClassifier(
        input_size=67,
    ...
    for data_id in sorted(os.listdir(args.dataset_path)):
        ...
        dataset = BackhandDataset(test_features)

        # Inference
        df_round_head = pd.DataFrame(columns=["ShotSeq", "Backhand"])
        model.eval()
        with torch.no_grad():
        ...
        # Save the result
        csv_path = os.path.join(data_path, f"{data_id}_backhand.csv")
