...
def predict(video_name, load_weights):
    ...
    f = open(video_name[:-4] + "_predict.csv", "w")
    ...
    out = cv2.VideoWriter(
        video_name[:-4] + "_predict" + video_name[-4:], fourcc, fps, size
    )
...
if __name__ == "__main__":
    try:
        (opts, args) = getopt.getopt(sys.argv[1:], "", ["video_name=", "load_weights="])
        if len(opts) != 2:
            raise ""
    except:
        print(
            "usage: python3 predict3.py --video_name=<videoPath> --load_weights=<weightPath>"
        )
        exit(1)
    ...
    predict(video_name, load_weights)
